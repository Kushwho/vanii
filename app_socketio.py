...
socketio = SocketIO(app_socketio, cors_allowed_origins=cors, async_mode='eventlet', ping_timeout=60000, ping_interval=25000, engineio_logger=True)
...
def initialize_deepgram_connection(sessionId, voice):
    ...
    options = LiveOptions(model="nova-2", language="en-IN", filler_words=True, smart_format=True, no_delay=True, keywords=["vaanii:7"], numerals=True,vad_events=True,utterance_end_ms='1000',interim_results=True,endpointing=200)
...
def handle_audio_stream(data):
    ...
    if sessionId in dg_connections:
        logging.info(f"Audio received for sessionId {sessionId}")
